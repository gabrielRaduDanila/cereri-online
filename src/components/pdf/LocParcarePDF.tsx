'use client'

import { Document, Page, Text, View, Image } from '@react-pdf/renderer'
import { inregistreazaFonturi } from './fonts'
import { stiluriComune, fmtData, gol, subsemnat, domiciliatGen } from './comun'

inregistreazaFonturi()

export type LocParcareData = {
  gen: 'M' | 'F' | ''
  numeSolicitant: string
  cnp: string
  adresa: string
  primarie: string
  numarAuto: string
  marcaModel: string
  oras: string
  dataDocument: string
  semnaturaDataUrl?: string | null
}

export default function LocParcarePDF({ data }: { data: LocParcareData }) {
  return (
    <Document>
      <Page size="A4" style={stiluriComune.page}>
        <View style={stiluriComune.antet}>
          <Text>Către,</Text>
          <Text>{gol(data.primarie, 30)}</Text>
        </View>

        <Text style={stiluriComune.titlu}>
          CERERE PENTRU ATRIBUIRE LOC DE PARCARE DE REȘEDINȚĂ
        </Text>

        <Text style={stiluriComune.paragraf}>
          {subsemnat(data.gen)} {gol(data.numeSolicitant, 30)}, având CNP{' '}
          {gol(data.cnp, 13)}, {domiciliatGen(data.gen)} în{' '}
          {gol(data.adresa, 40)}, prin prezenta vă rog să-mi aprobați
          atribuirea unui loc de parcare de reședință în zona în care
          locuiesc.
        </Text>

        <Text style={stiluriComune.paragraf}>
          Autoturismul pentru care solicit locul de parcare:{' '}
          {gol(data.marcaModel, 20)}, având numărul de înmatriculare{' '}
          {gol(data.numarAuto, 10)}.
        </Text>

        <Text style={stiluriComune.paragraf}>
          Anexez prezentei: copie act identitate, copie certificat de
          înmatriculare auto, dovada plății taxei (dacă este cazul).
        </Text>

        <Text style={stiluriComune.paragraf}>
          Vă mulțumesc.
        </Text>

        <View style={stiluriComune.semnaturaRand}>
          <View style={stiluriComune.semnaturaCol}>
            <Text style={stiluriComune.semnaturaEticheta}>Data:</Text>
            <Text>{fmtData(data.dataDocument)}</Text>
            <Text style={{ marginTop: 10 }}>
              Localitatea: {gol(data.oras, 15)}
            </Text>
          </View>
          <View style={stiluriComune.semnaturaCol}>
            <Text style={stiluriComune.semnaturaEticheta}>Semnătura,</Text>
            {data.semnaturaDataUrl ? (
              <Image
                src={data.semnaturaDataUrl}
                style={stiluriComune.semnaturaImg}
              />
            ) : (
              <Text>____________________</Text>
            )}
          </View>
        </View>
      </Page>
    </Document>
  )
}
